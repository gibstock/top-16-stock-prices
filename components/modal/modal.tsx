...
export const Modal = ({curr, setCurrentSymbol, setTickerSymbols, tickerSymbols, setLaunchModal, listOfSymbols}: AppProps) => {
  ...
  return (
    <>
      <div className='fixed flex flex-col justify-center items-center p-2 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/2  rounded-lg shadow-[0px_1px_4px] shadow-light-shadow z-10 dark:shadow-dark-shadow'>
        <form className='flex flex-col justify-center items-center' onSubmit={handleAddQuoteClick}>
          <input 
            className=' text-xl  px-3 py-2 rounded-xl'
            name='newQuote'
            type='text' 
            placeholder={curr}
            autoFocus={true}
          />
          <button className='bg-blue-500 px-3 py-2 mt-2 rounded-full w-full text-light-primary5 font-bold text-xl' type='submit'>Add Quote</button>
        </form>
      </div>
